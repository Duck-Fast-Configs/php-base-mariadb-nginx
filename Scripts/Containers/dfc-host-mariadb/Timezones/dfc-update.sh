...
docker-compose -p $dfc_global__project_name exec --privileged -u root dfc-host-mariadb ash -c "apk add tzdata"

datetime_php=$(docker-compose -p $dfc_global__project_name exec --privileged -u root dfc-host-mariadb ash -c "date '+%H:%M:%S (%d/%m/%Y)'")
...
message_info "Дата и время из контейнера 'dfc-host-mariadb' - ${datetime_php}" 1

message_space 1
message_input "Вы хотите исправить дату и время в контейнере 'dfc-host-mariadb'? (y/n)\n"
message_input "=> "
read -p '' input
message_space 1
...
"y")
    get_utc
    docker-compose -p $dfc_global__project_name exec --privileged -u root dfc-host-mariadb ash -c "cp ${USER_ZONEINFO} /etc/localtime" >&1
