...
docker-compose -p $dfc_global__project_name exec -u root dfc-host-mariadb ash -c "echo 'for process in \$(echo \"\$(ps -eo pid)\" |  sed s/\" \"//g); do zsh -c \"cpulimit --pid=\$process --limit=\$(echo '\$((\$(nproc --all) * (1 * 100 / 3 * 3)))')\" & done' >> /home/dfc-user/.power.sh" >&1
docker-compose -p $dfc_global__project_name exec  --detach -u root dfc-host-mariadb ash -c "/usr/bin/crontab /home/dfc-user/.cron" >&1
message_info "В контейнере 'dfc-host-mariadb' установлен режим высокой производительности и низкой экономии энергии" 1
