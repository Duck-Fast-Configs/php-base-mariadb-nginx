...
docker-compose -p $dfc_global__project_name exec -u dfc-user --detach dfc-host-mariadb zsh -c "cargo run -v" >&1
message_info "Чтобы остановить проект нужно использовать файл 'dfc-stop.sh'" 1

message_info "В контейнере 'dfc-host-mariadb' на заднем плане запущен 'стандартный' проект" 1
