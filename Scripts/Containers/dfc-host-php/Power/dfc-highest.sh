...
docker-compose -p $dfc_global__project_name exec --privileged -u root dfc-host-php ash -c "echo 'for process in \$(echo \"\$(ps -eo pid)\" |  sed s/\" \"//g); do zsh -c \"cpulimit --pid=\$process --limit=\$(echo '\$((\$(nproc --all) * 100))')\" & done' >> /home/dfc-user/.power.sh" >&1
...
message_info "В контейнере 'dfc-host-php' установлен режим максимальной производительности и минимальной экономии энергии" 1
